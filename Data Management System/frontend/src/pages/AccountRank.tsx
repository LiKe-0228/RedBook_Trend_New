...
import { api, AccountRankItem, ListResponse } from "../api/client";
...
export default function AccountRankPage() {
  const [form] = Form.useForm<{
    q?: string;
    fetch_date_range?: [Dayjs, Dayjs];
  }>();
  const [data, setData] = useState<AccountRankItem[]>([]);
  ...
  const handleColumnSearch = useCallback(
    (
      dataIndex: keyof AccountRankItem,
      value: string,
      confirm: FilterDropdownProps<AccountRankItem>["confirm"]
    ) => {
  ...
  const getColumnSearchProps = useCallback(
    (dataIndex: keyof AccountRankItem) => ({
      filterDropdown: ({
        ...
        clearFilters,
        close
      }: FilterDropdownProps<AccountRankItem>) => (
      ...
      onFilter: (value: string | number | boolean, record: AccountRankItem) => {
  ...
  const columns: ColumnsType<AccountRankItem> = useMemo(
    () => [
      {
        title: "排名",
        dataIndex: "rank",
      ...
      },
      {
        ...
        width: 100,
        ...getColumnSearchProps("fans_count")
      },
      {
        title: "阅读数",
  ...
  const fetchData = async (
    ...
    setLoading(true);
    try {
      const resp = await api.get<ListResponse<AccountRankItem>>("/account_rank", {
        params: {
          page,
          page_size: pageSize,
          q: nextFilters.q,
      ...
      console.error(err);
      message.error(err?.message || "加载账号榜失败");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Space direction="vertical" size="middle" style={{ width: "100%" }}>
      ...
      >
        <Form.Item label="关键词" name="q">
          <Input allowClear placeholder="店铺名 模糊搜索" style={{ width: 220 }} />
        </Form.Item>
